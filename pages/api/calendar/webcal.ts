...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // If token is provided, validate it and get user preferences
    let userPreferences = {
    ...
    if (token) {
      // Validate subscription token and get user preferences
      ...
      if (subscription.user_calendar_preferences) {
        const prefs = subscription.user_calendar_preferences;
        userPreferences = {
          include_auspicious: prefs.include_auspicious_times,
          include_inauspicious: prefs.include_inauspicious_times,
          include_special_days: prefs.include_special_days,
          include_rs_warnings: prefs.include_rs_warnings,
          include_chandrashtama: prefs.include_chandrashtama,
          calendar_name: prefs.calendar_name
        };

        // Use user's preferred date range
        const rangeDays = prefs.date_range_days || 365;
        const endDate = new Date(new Date(start_date as string).getTime() + rangeDays * 24 * 60 * 60 * 1000);
    ...
    // Generate ICS content with user preferences
    const icsContent = generateICSContent(panchangamData, userPreferences);

    // Set appropriate headers for webcal subscription
    res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
    res.setHeader('Content-Disposition', `inline; filename="${userPreferences.calendar_name}.ics"`);
    ...
    console.error('Error generating webcal calendar:', error);
...
X-WR-RELCALID:${calendar_name.replace(/\s+/g, '')}
